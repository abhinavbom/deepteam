...
bfla = BFLA(types=["function_bypass", "function_chaining_escalation"])
...
- [Optional] `types`: a list of `types` of `BFLA` to test through. Defaulted to all `types` available. Here are the list of `types` available for `BFLA`:
  - `function_bypass`: Tests whether the agent refuses to directly invoke admin, internal, management, or maintenance functions and restricted endpoints when they are framed as routine operations.
  - `authorization_bypass`: Tests whether the agent enforces a function's own authorization check — e.g. refusing a write or mutation path while the corresponding read is gated, or refusing an over-permissioned generic tool asked to perform privileged work.
  - `function_chaining_escalation`: Tests whether the agent prevents chaining individually-authorized functions, or pivoting to a broader-privileged sibling function or agent, to reach a privileged outcome.
  - `legacy_function_exploitation`: Tests whether the agent blocks deprecated, legacy, or alternate-version functions and shadow endpoints that predate the current authorization checks.
...
bfla = BFLA(
    types=["function_bypass", "function_chaining_escalation"],
